...
function prettyBreadcrumbs(breadcrumbs, place) {
  var result = '';

  result += (place ? colors.errorPlace(place + ':' + '\n') : '' );

  _.take(_.reverse(breadcrumbs), 3).forEach(function(breadcrumb) {
    result += breadcrumb.message + '\n';
    if (breadcrumb.trace) {
      breadcrumb.trace
        .split(/\n/)
        .map(dedentTraceLine)
...
exports.prettyBreadcrumbs = prettyBreadcrumbs;
