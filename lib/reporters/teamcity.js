...
Teamcity.prototype.gotMessage = function(testPath, message) {
  ...
  if (message.type === 'stdout') {
    ...
    var breadcrumbs = this._breadcrumbTracker.getMessages(testPath);
    if (breadcrumbs.length > 0) {
      var breadcrumbsString = errorMessageUtil.prettyBreadcrumbs(breadcrumbs, 'Last breadcrumb(s)');
      msgs.push(errorMessageUtil.indent(breadcrumbsString, indentation));
    }

    var breadcrumb = _.last(breadcrumbs);
    var timeoutMessage = {
      name: testName,
