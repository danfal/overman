...
ErrorDetail.prototype.done = function() {
  ...
  this._failures.forEach(function(failure, idx) {
    ...
    var shouldShowBreadcrumbs = (
      failure.message.result === 'timeout' ||
      errors.length !== 0 && _.last(errors).in === 'uncaught');

    var breadcrumbs = self._breadcrumbTracker.getMessages(failure.test);
    if (breadcrumbs.length > 0 && shouldShowBreadcrumbs) {
      var breadcrumbString = errorMessageUtil.prettyBreadcrumbs(breadcrumbs, 'Last breadcrumb(s)');
